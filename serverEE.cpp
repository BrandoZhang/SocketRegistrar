...
const string COURSE_FILE = "ee.txt";
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "The ServerEE is up and running using UDP on port " << PORT_SERVER_EE << "." << std::endl;
    ...
    if (course_file.is_open()) {
        ...
        std::cerr << "ERROR: serverEE cannot open file `" << COURSE_FILE << "`!" << std::endl;
        exit(-1);
    }
    ...
    // Setup UDP server
    int sockfd;
    setup_server_socket(sockfd, SOCK_DGRAM, PORT_SERVER_EE);
    ...
    while (true) {
        ...
        std::cout << "The ServerEE received a request from the Main Server about the " << category << " of " << course_code << "." << std::endl;
